...
export async function updateArtifacts(opts: ReleaseOpts) {
	...
	const awsAccessKeyId =
		...
		(await $`op read "op://Engineering/tivet-releases R2 Upload/username"`.text());
	const awsSecretAccessKey =
		Deno.env.get("R2_RELEASES_SECRET_ACCESS_KEY") ??
		(await $`op read "op://Engineering/tivet-releases R2 Upload/password"`.text());
	...
	// List all files under tivet/{commit}/
	const commitPrefix = `tivet/${opts.commit}/`;
	$.logStep("Listing Original Files", commitPrefix);
	const commitFiles = await awsCommand
		.command(
			`aws s3api list-objects --bucket tivet-releases --prefix ${commitPrefix}`,
		)
		.json();
	assert(
		Array.isArray(commitFiles?.Contents) && commitFiles.Contents.length > 0,
		`No files found under tivet/${opts.commit}/`,
	);

	// Copy files to version directory
	const versionTarget = `tivet/${opts.version}/`;
	...
	if (opts.latest) {
		await copyFiles(awsCommand, commitPrefix, "tivet/latest/");
...
async function copyFiles(
	...
	await $.logGroup(async () => {
		...
		await awsCommand
			.command(`aws s3 rm s3://tivet-releases/${targetPrefix} --recursive`)
			.spawn();
		...
		await awsCommand
			.command(
				`aws s3 cp s3://tivet-releases/${sourcePrefix} s3://tivet-releases/${targetPrefix} --recursive --copy-props none`,
			)
			.spawn();
	});
}
...
async function generateInstallScripts(
	...
	for (const scriptPath of installScriptPaths) {
		...
		const uploadKey = `tivet/${version}/${scriptPath.split("/").pop() ?? ""}`;

		// Upload the install script to S3
		$.logStep("Uploading Install Script", uploadKey);
		await awsCommand
			.command(`aws s3 cp - s3://tivet-releases/${uploadKey}`)
