...
rm -rf /tmp/tivet_cli_install
mkdir /tmp/tivet_cli_install
cd /tmp/tivet_cli_install

TIVET_CLI_VERSION="__VERSION__"
UNAME="$(uname -s)"
ARCH="$(uname -m)"
...
if [ "$(printf '%s' "$UNAME" | cut -c 1-6)" = "Darwin" ]; then
	...
	if [ "$ARCH" = "x86_64" ]; then
		FILE_NAME="tivet-x86-mac"
	elif [ "$ARCH" = "arm64" ]; then
		FILE_NAME="tivet-aarch64-mac"
	else
...
# Download CLI
URL="https://releases.tivet.gg/tivet/${TIVET_CLI_VERSION}/${FILE_NAME}"
echo
echo "> Downloading $URL"
curl -fsSL "$URL" -o tivet-cli
chmod +x tivet-cli

# Move binary
if [ ! -w "$BIN_DIR" ]; then
    echo
    echo "> Installing tivet to $INSTALL_PATH (requires sudo)"
    sudo mv ./tivet-cli "$INSTALL_PATH"
else
    echo
    echo "> Installing tivet to $INSTALL_PATH"
    mv ./tivet-cli "$INSTALL_PATH"
fi
...
"$BIN_DIR/tivet" --version

echo
echo "Tivet was installed successfully."
echo "Run 'tivet --help' to get started."
